...
async function loadOrders(){const {data,error}=await sb.from('store_orders').select('*').order('created_at',{ascending:false}).limit(100);if(error){A('#ordersBody').innerHTML=`<tr><td colspan="5">Run SUPABASE_CHECKOUT.sql first.</td></tr>`;return}A('#ordersBody').innerHTML=(data||[]).map(o=>`<tr><td>${esc(o.order_number)}</td><td>${esc(o.customer_name)}<br>${esc(o.phone)}</td><td>${Number(o.total).toLocaleString()} EGP</td><td><select onchange="setStatus('${o.id}',this.value)">${['new','confirmed','preparing','shipped','completed','cancelled'].map(s=>`<option ${o.status===s?'selected':''}>${s}</option>`).join('')}</select></td><td>${new Date(o.created_at).toLocaleString()}</td></tr>`).join('')}
...
async function saveAnnouncement(e){e.preventDefault();const id=A('#annId').value,row={text_ar:A('#annAr').value,text_en:A('#annEn').value,active:A('#annActive').checked};const r=id?await sb.from('announcements').update(row).eq('id',id):await sb.from('announcements').insert(row);if(r.error)alert(r.error.message);else alert('Saved')}
...
async function loadEvents(){
  ...
  if(error){body.innerHTML='<tr><td colspan="7">Run SUPABASE_EVENT_CART.sql first.</td></tr>';return}
  body.innerHTML=(data||[]).map(r=>`<tr>
    ...
    <td>${r.guest_count??'—'}</td>
    <td><select onchange="setEventStatus('${r.id}',this.value)">${['new','contacted','planning','quoted','confirmed','completed','cancelled'].map(s=>`<option ${r.status===s?'selected':''}>${s}</option>`).join('')}</select></td>
